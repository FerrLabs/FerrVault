...
| `reconcileTimeout` | `2m` | A single reconcile is cancelled after this long, so one call that never returns cannot hold the work queue. Keep it well above the 10s FerrVault API client timeout. |
